...
class SaleController extends Controller
{
    ...
    public function saleData(Request $request)
    {
        ...
        if (!empty($sales)) {
            foreach ($sales as $key => $sale) {
                ...
                $nestedData['document'] = $sale->document
                ? '<img src="' . asset('documents/sale/' . $sale->document) . '" alt="Image" height="50">'
                : 'No Image';
                $nestedData['image'] = $sale->image
                    ? '<img src="' . asset('images/sale/' . $sale->image) . '" alt="Image" height="50">'
                    : 'No Image';

                // Thêm cột delivery_date
                $nestedData['delivery_date'] = $sale->delivery_date
    ...
    public function create()
    {
        ...
        if ($role->hasPermissionTo('sales-add')) {
            ...
            $currency_list = Currency::where('is_active', true)->orderBy('id', 'desc')->get();
    ...
    public function store(Request $request)
    {
        ...
        if ($document) {
            ...
            // Gán tài liệu mới vào dữ liệu
            $data['document'] = $documentName;
        } else {

            if ($request->current_document) {
                $currentImagePath = public_path('images/quotation/' . $request->current_document); // Đường dẫn hình ảnh hiện tại
                $newImagePath = public_path('documents/sale/' . $request->current_document); // Đường dẫn lưu mới

                // Kiểm tra nếu hình ảnh hiện tại tồn tại và sao chép
                if (file_exists($currentImagePath)) {
                    if (!file_exists(public_path('documents/sale'))) {
                        mkdir(public_path('documents/sale'), 0755, true); // Tạo thư mục nếu chưa tồn tại
                    }
                    copy($currentImagePath, $newImagePath);
                }
                $data['document'] = $request->current_document; // Gán lại hình ảnh hiện tại
            }
        }
        ...
        if ($image) {
            ...
            if (!config('database.connections.saleprosaas_landlord')) {
                ...
                $image->move(public_path('images/sale'), $imageName);
            } else {
                $imageName = $this->getTenantId() . '_' . $imageName . '.' . $ext;
                $image->move(public_path('images/sale'), $imageName);
            }

            // Gán hình ảnh mới vào dữ liệu
            $data['image'] = $imageName;
        } else {
            // Xử lý sao chép hình ảnh hiện tại vào đường dẫn đích
            if ($request->current_image) {
                $currentImagePath = public_path('images/quotation/' . $request->current_image); // Đường dẫn hình ảnh hiện tại
                $newImagePath = public_path('images/sale/' . $request->current_image); // Đường dẫn lưu mới

                // Kiểm tra nếu hình ảnh hiện tại tồn tại và sao chép
                if (file_exists($currentImagePath)) {
                    if (!file_exists(public_path('images/sale'))) {
                        mkdir(public_path('images/sale'), 0755, true); // Tạo thư mục nếu chưa tồn tại
                    }
                    copy($currentImagePath, $newImagePath);
                }
                $data['image'] = $request->current_image; // Gán lại hình ảnh hiện tại
            }
        }
        ...
        if ($mail_data['email'] && $data['sale_status'] == 1 && $mail_setting) {
            ...
            try {
                FacadesMail::to($mail_data['email'])->send(new SaleDetails($mail_data));
