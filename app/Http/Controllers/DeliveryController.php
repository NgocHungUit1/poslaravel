...
class DeliveryController extends Controller
{
    ...
    public function create($id)
    {
        ...
        if ($lims_delivery_data) {
            ...
            $returned_amount = DB::table('returns')
                ->where('sale_id', $customer_sale->sale_id)
                ->sum('grand_total');

            $grand_total = number_format($customer_sale->grand_total, 0, ',', '.');
            $returned_amount = number_format($returned_amount, 0, ',', '.');
            $paid_amount = number_format($customer_sale->paid_amount, 0, ',', '.');
            $due = $customer_sale->grand_total - $returned_amount - $customer_sale->paid_amount;
            ...
            $delivery_data[] = $customer_sale->name;
        ...
        $due = number_format($due, 0, ',', '.');
            ...
            $delivery_data[] = '';
            $delivery_data[] = $customer_sale->name;
            $delivery_data[] = $customer_sale->phone_number;
            $delivery_data[] = $customer_sale->name;
            $delivery_data[] = $customer_sale->address . ' ' . $customer_sale->city . ' ' . $customer_sale->country;
            $delivery_data[] = '';
            $delivery_data[] = '';
            $delivery_data[] = $due;
        }
